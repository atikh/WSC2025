        ...
        for dimension, change_type, value in transition.dimension_changes:
            if dimension in transition.dimension_table:
                if change_type == "fixed":
                    ...
                    print("HERE",transition.label,'VALUE', value,'duration',transition.firing_delay, "Fur this",value*transition.firing_delay,"total",transition.dimension_table[dimension])
            else:
            ...
            if tracking_entry:
                ...
                for dimension, change_type, value in transition.dimension_changes:
                    if dimension == tracked_dimension and change_type == "rate":
                        calculated_change = value * duration * transition.capacity
                        transition.dimension_table[dimension] += calculated_change
                    else:
                        transition.dimension_table[ dimension] = value * duration * transition.capacity
                    # Check if input places are DoT and calculate the duration
...
############################
    ...
    Ouputt=0
    for transition in spn.transitions:
        if transition.label== "Preprocessing1 (Begin)":
            C1_Elctricity = C1_Elctricity+ transition.dimension_table['Energy']
        if transition.label== "Processing1 (Begin)":
            C1_Elctricity = C1_Elctricity + transition.dimension_table['Energy']
            Waste_Cell1_Pls = transition.dimension_table['Plastic Waste']
        if transition.label== "Preprocessing2 (Begin)":
            C2_Elctricity = C2_Elctricity+transition.dimension_table['Energy']
        if transition.label== "Processing2 (Begin)":
            C2_Elctricity = C2_Elctricity + transition.dimension_table['Energy']
            Waste_Cell2_Pls = transition.dimension_table['Plastic Waste']
        if hasattr(transition, 'input_value'):
            Inputt = transition.input_value
        if hasattr(transition, 'output_value'):
            Ouputt = Ouputt + transition.output_value
    print("KPIS",C1_Elctricity,C2_Elctricity, C1_Elctricity+C2_Elctricity,Waste_Cell1_Pls,
          Waste_Cell2_Pls,Waste_Cell1_Pls+Waste_Cell2_Pls,Inputt,Ouputt, Ouputt/Inputt)
    calculate_and_write_kpis(spn)
#############################

def write_kpis_to_csv(data, path="../output/KPIs/KPIs.csv"):
...
def calculate_and_write_kpis(spn):
    # Example data extraction logic, you need to adjust it based on your actual simulation results
    C1_Electricity = sum(transition.dimension_table.get('Energy', 0) for transition in spn.transitions if transition.label in ["Preprocessing1 (Begin)", "Processing1 (Begin)"])
    C2_Electricity = sum(transition.dimension_table.get('Energy', 0) for transition in spn.transitions if transition.label in ["Preprocessing2 (Begin)", "Processing2 (Begin)"])
    Waste_Cell1_Pls = sum(transition.dimension_table.get('Plastic Waste', 0) for transition in spn.transitions if transition.label == "Processing1 (Begin)")
    Waste_Cell2_Pls = sum(transition.dimension_table.get('Plastic Waste', 0) for transition in spn.transitions if transition.label == "Processing2 (Begin)")
    Inputt = sum(getattr(transition, 'input_value', 0) for transition in spn.transitions)
    Ouputt = sum(getattr(transition, 'output_value', 0) for transition in spn.transitions)

    # Calculate the KPIs
    Total_Electricity = C1_Electricity + C2_Electricity
    Total_Waste_Pls = Waste_Cell1_Pls + Waste_Cell2_Pls
    Output_to_Input_Ratio = Ouputt / Inputt if Inputt else 0  # Prevent division by zero

    # Prepare data for CSV
    data = [C1_Electricity, C2_Electricity, Total_Electricity, Waste_Cell1_Pls, Waste_Cell2_Pls, Total_Waste_Pls, Inputt, Ouputt, Output_to_Input_Ratio]

    # Write to CSV
    write_kpis_to_csv(data)